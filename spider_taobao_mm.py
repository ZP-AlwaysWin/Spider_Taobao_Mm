...
class Spider(object):
    ...
    def getDetailPage(self,infoURL):
        infoURL="https:"+str(infoURL)
        print(infoURL)
        request=urllib.request.Request(infoURL)
        request.add_header("User-Agent","Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3100.0 Safari/537.36")
        response=urllib.request.urlopen(request)
    ...
    def getAllImg(self,page):
        pattern = re.compile('<div class="mm-aixiu-content".*?>(.*?)<!--爱秀内容结束->',re.S)
        #个人信息页面所有代码
